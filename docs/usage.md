...
```bash
nextflow pull avantonder/assembleBAC
```
...
First, go to the [avantonder/assembleBAC releases page](https://github.com/avantonder/assembleBAC/releases) and find the latest version number - numeric only (eg. `v2.0.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r v2.0.1`.
